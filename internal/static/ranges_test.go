...
// filmTag is the entity tag every response in this file carries, which is the hash of those
// thousand octets. Computed rather than written out, unlike the three fixtures in static_test.go:
// the content is generated by a loop, so a literal here would say nothing a reader could check
// and everything it did say would be about the loop.
var filmTag = tagOf(content(filmSize))
...
// partial206 is the header section of a single-part 206: the eight fields a 200 on a file has,
// and the content-range after them.
func partial206(kind, tag string, length int, contentRange string) []h2.Field {
	return []h2.Field{
...
//
// The entity tag is the hash of the whole file rather than of the parts, which is the point of
// the field on a 206: §14.4 of RFC 9110 makes the validator describe the selected representation,
// so a peer can join two partial responses only because both name the same representation.
...
func TestRangeSinglePart(t *testing.T) {
	...
	assertFields(t, a, partial206(filmType, filmTag, 10, "bytes 0-9/1000"))
	if a.body != body[0:10] {
...
func TestRangeWholeFileIsStillPartial(t *testing.T) {
	...
	for _, value := range []string{"bytes=0-999", "bytes=0-", "bytes=-1000", "bytes=0-" + huge} {
		...
		assertFields(t, a, partial206(filmType, filmTag, filmSize, "bytes 0-999/1000"))
		if a.body != body {
...
func TestRangeOnDirectoryIndex(t *testing.T) {
	...
	assertFields(t, a, partial206("text/html; charset=utf-8", pageTag, 10,
...
func TestRangeOneSatisfiableSpecOfManyIsSinglePart(t *testing.T) {
	...
	for _, c := range []struct {
		...
		length := c.last - c.first + 1
		want := "bytes " + strconv.Itoa(c.first) + "-" + strconv.Itoa(c.last) + "/1000"
		assertFields(t, a, partial206(filmType, filmTag, length, want))
		if a.body != body[c.first:c.last+1] {
...
func TestRangeStopsCountingAtTheBound(t *testing.T) {
	...
	assertFields(t, a, ok(filmType, filmTag, filmSize))
	if a.body != body {
...
func TestRangeEmptyElementsAreParsedAndNotCounted(t *testing.T) {
	...
	for _, value := range []string{"bytes=", "bytes=,", "bytes=,,,,", "bytes=" + strings.Repeat(",", 10000)} {
		...
		assertFields(t, b, ok(filmType, filmTag, filmSize))
		if b.body != body {
...
func TestRangeListWhitespaceIsAccepted(t *testing.T) {
	...
	a := rangeGet(t, h, "/"+filmName, "bytes= 0-9")
	assertFields(t, a, partial206(filmType, tagOf(body), 10, "bytes 0-9/6000"))
}
...
func TestRangeOverflowingNumeralIsNotASyntaxError(t *testing.T) {
	...
	assertFields(t, a, partial206(filmType, filmTag, 1, "bytes 0-0/1000"))
	if a.body != body[0:1] {
	...
	assertFields(t, b, partial206(filmType, filmTag, 5, "bytes 995-999/1000"))
	...
	assertFields(t, c, partial206(filmType, filmTag, filmSize, "bytes 0-999/1000"))
...
func TestRangeSumAtTheRepresentationIsPartial(t *testing.T) {
	...
	assertFields(t, b, partial206(filmType, filmTag, 10, "bytes 0-9/1000"))
}
...
func TestRangeIgnoredOnEmptyFile(t *testing.T) {
	...
	for _, value := range []string{"bytes=0-0", "bytes=-1", "bytes=0-", "bytes=-0", "bytes=abc"} {
		a := rangeGet(t, h, "/"+filmName, value)
		assertFields(t, a, ok(filmType, emptyTag, 0))
		if a.body != "" {
...
func TestPartialShrankEndsTheStreamFirst(t *testing.T) {
	...
	fields := partial206(filmType, filmTag, int(over.length()), over.contentRange(filmSize+10))
