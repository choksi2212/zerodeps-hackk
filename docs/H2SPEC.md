...
`h2spec` is an HTTP/2 conformance suite that speaks the protocol at a server and
checks what comes back against RFC 9113 and RFC 7541, clause by clause. It is an
external checker and not a dependency: it is a separate binary that talks to
`zdh` over a TCP socket, so nothing about it appears in this module's build. The
zero-dependency claim in the README is unaffected by it, and is checked by
`scripts/gate.sh` independently.

It is here because it is the one measurement in this repository that the authors
did not design. Every test in `go test ./...` was written by whoever wrote the
code it covers, and every break in `scripts/break-*.py` was written by whoever
wrote the guard it removes. `h2spec` was written by neither, before this project
existed, from the RFC rather than from this implementation.

## Result

**147 tests, 147 passed, 0 skipped, 0 failed** — with `--strict`, which enables
the cases where the RFC permits a choice and h2spec holds the server to the
stricter reading.

```bash
bin/zdh -dir public                       # in one terminal
h2spec -h 127.0.0.1 -p 8081 --strict --timeout 5
```

Run against the cleartext port, which serves h2c by prior knowledge (§3.3): the
peer sends the connection preface immediately and there is no upgrade to
negotiate, which is what h2spec expects. The TLS port serves the same server
behind the same accept loop, so what is measured here is not particular to the
cleartext listener.

- `h2spec` version 2.6.0 (`70ac2294010887f48b18e2d64f5cccd48421fad1`)
- `zdh` version 0.1.0-dev, built by `scripts/build.sh`

The suite has three parts, and the transcript below is in their order. The
generic tests exercise a server the way a client does. The RFC 9113 tests cover
§3.5 (the connection preface), §4.1--§4.3 (frame format, frame size, and header
compression), §5.1 and §5.3--§5.5 (stream states, priority, error handling, and
extension frames), §6.1--§6.5 and §6.7--§6.10 (nine of the ten frame types), §7
(error codes), and §8.1--§8.2 (message exchange, pseudo-header fields, malformed
requests, and server push). The RFC 7541 tests cover §2.3.3 (the index address
space), §4.2 (maximum table size), §5.2 (string literals, including Huffman
padding and the EOS symbol), and §6.1 and §6.3 (indexed representations and the
dynamic table size update).

Two things h2spec does not measure, named here rather than left to be noticed.
There is no §6.6 test, because PUSH_PROMISE is a frame a server sends and h2spec
only checks that this server refuses to receive one, which it does under §8.2.
And §5.2's connection-level flow control has no section of its own in the suite —
the credit arithmetic is exercised by the DATA and WINDOW_UPDATE tests under §6.1
and §6.9, and its own coverage is `internal/flow`'s 60 tests and 92 breaks.
