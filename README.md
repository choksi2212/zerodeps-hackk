...
Each of the four rules has been observed failing by name: the §6.5.2 overhead put back; a span whose two fragments appear in §6.2 in the wrong order; the one span in the tree that no `§` introduces, corrupted a word at a time; and a short quoted span planted in front of a misquotation, which the regex version reported as clean and this one names. A path given explicitly and not found exits 2 rather than 1, so a mistyped path cannot be mistaken for a finding, and a checkout with no RFC beside it reports a skip and stays green.
...
python scripts/break-table.py    # 128 breaks, all 128 caught
