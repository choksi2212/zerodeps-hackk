...
| Streams and flow control (§5) | working — 145 tests, and 223 guards each observed failing; the send half of flow control is not yet wired to the stream table, and none of it is reachable from `cmd/zdh` |
| HPACK (RFC 7541) | in progress, separate author |
| Request semantics (RFC 9113 §8) | working — 70 tests, and 132 guards each observed failing; response writing and the static file handler are not started |
